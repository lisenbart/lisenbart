...
export default function StudioPopover({ clickPoint, rootRef, onClose }: StudioPopoverProps) {
  const panelRef = useRef<HTMLDivElement>(null);
  const copy = site.studioModal;
  ...
  return createPortal(
    <motion.div
      ref={panelRef}
      role="dialog"
      aria-labelledby="studio-popover-title"
      tabIndex={-1}
      initial={{ opacity: 0, scale: 0.96 }}
      animate={{ opacity: isReady ? 1 : 0, scale: isReady ? 1 : 0.96 }}
      exit={{ opacity: 0, scale: 0.98 }}
      transition={{ type: "spring", stiffness: 420, damping: 32, mass: 0.82 }}
      className="studio-popover clients-popover capability-showreel-modal modal-panel fixed z-[100] flex max-h-[min(72vh,28rem)] flex-col overflow-hidden rounded-[20px] outline-none"
      style={{
