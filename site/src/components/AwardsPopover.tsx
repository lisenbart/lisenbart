...
export default function AwardsPopover({ clickPoint, rootRef, onClose }: AwardsPopoverProps) {
  const panelRef = useRef<HTMLDivElement>(null);
  const copy = site.awardsModal;
  ...
  return createPortal(
    <motion.div
      ref={panelRef}
      role="dialog"
      aria-labelledby="awards-popover-title"
      tabIndex={-1}
      initial={{ opacity: 0, scale: 0.96 }}
      animate={{ opacity: isReady ? 1 : 0, scale: isReady ? 1 : 0.96 }}
      exit={{ opacity: 0, scale: 0.98 }}
      transition={{ type: "spring", stiffness: 420, damping: 32, mass: 0.82 }}
      className="awards-popover studio-popover clients-popover capability-showreel-modal modal-panel fixed z-[100] flex max-h-[min(72vh,28rem)] flex-col overflow-hidden rounded-[20px] outline-none"
      style={{
    ...
    >
      ...
      <div className="relative flex min-h-0 flex-1 flex-col px-5 pb-5 pt-6">
        ...
        <div className="mt-4 shrink-0">
          <a
            href={`${workCategoryHref("film")}#unnecessary-things`}
